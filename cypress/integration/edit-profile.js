describe("Edit Profile test", () => {
    ...
    it("Edit User Profile", () => {
        ...
        cy.get(".available-photos .photo-item")
            .first()
        ...
        // edit profile
        cy.get("input[name=Location]").type("Philadelphia, PA").tab();

        cy.focused().should("have.attr", "name", "About").type("Meow").tab();
        ...
        cy.focused().should("contain", "Save Profile").tab();

        cy.focused().should("have.attr", "name", "Email").type("email@example.com").tab();

        cy.focused().should("have.attr", "name", "Phone").type("(123) 456-7890{enter}");
        ...
        cy.request("/profile?format=json")
            .its("body.data")
            .then((data) => {
