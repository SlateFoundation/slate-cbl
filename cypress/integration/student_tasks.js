...
describe.skip('Student tasks test', () => {
    ...
    it('Re-Assign task as teacher', () => {
        ...
        cy.withExt().then(({ extQuerySelector }) => {
            ...
            cy.get('.slate-window').contains('label', 'Re-assign').click({ force: true });
            ...
            cy.get('#' + currentTasksTree.id).contains('.slate-tasktree-status', 'Revision');
        });
    });
                ...
                .then(() => {
                    ...
                    cy.wait('@studentTasksData').should((xhr) => {
                        ...
                        cy.get('#' + currentTasksTree.id).should(($tasksTree) => {
                            expect($tasksTree.find('li.slate-tasktree-item').length).to.equal(1);
                        });
                    });
                                ...
                                cy.wait('@studentTasksData')
                                    ...
                                    .then(() => {
                                        ...
                                        cy.wait('@studentTasksData')
                                            ...
                                            .and(() => {
                                                expect(cy.get('.slate-tasktree-list').find('li.slate-tasktree-item').should('have.length', 9)).to.equal(length);
                                            });
                                    });
                            }
                        });
