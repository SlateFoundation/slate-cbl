describe("/cbl/student-competencies API", () => {
    ...
    it("Expected student competencies exist", () => {
        ...
        cy.request(
            "/cbl/student-competencies/1?format=json&include=completion,effectiveDemonstrationsData"
        ).then((response) => {
            expect(response).property("status").to.eq(200);
            expect(response).property("body").to.be.an("object");
            ...
            expect(response.body.data.effectiveDemonstrationsData).to.have.all.keys(
                "1",
                "2",
                "3",
                "4"
    ...
    it("Can change DemonstratedLevel on existing student task rating", () => {
        cy.request(
            "POST",
            "/cbl/student-tasks/save?format=json&include=Demonstration.DemonstrationSkills",
            {
                data: [
                    {
                        ID: 1,
            ...
            expect(response.body.data[0].Demonstration)
                .property("DemonstrationSkills")
                .to.be.an("array")
                .that.has.length(1);
    ...
    it("Validation prevents TargetLevel changes to existing StudentTask rating", () => {
        cy.request(
            ...
            expect(response.body).property(
                "message",
                "TargetLevel cannot be changed on existing records"
            );
            expect(response.body).property("success", false);
            expect(response.body).property("failed").to.be.an("array").that.has.length(1);
            expect(response.body.failed[0])
                .to.be.an("object")
                .that.has.all.keys("record", "validationErrors");
            expect(response.body.failed[0].record).to.be.an("object");
            expect(response.body.failed[0].validationErrors)
                .to.be.an("object")
                .that.has.all.keys("Demonstration");
            expect(response.body.failed[0].validationErrors.Demonstration)
                .to.be.an("object")
                .that.has.all.keys("DemonstrationSkills");
            expect(response.body.failed[0].validationErrors.Demonstration.DemonstrationSkills)
                .to.be.an("array")
                .that.has.length(1);
            expect(response.body.failed[0].validationErrors.Demonstration.DemonstrationSkills[0])
                .to.be.an("object")
                .that.has.property(
                    "TargetLevel",
                    "TargetLevel cannot be changed on existing records"
    ...
    it("Rating remains unchanged after invalid edit", () => {
        cy.request(
            "/cbl/student-tasks/1?format=json&include=Demonstration.DemonstrationSkills"
        ).then((response) => {
            expect(response).property("status").to.eq(200);
            expect(response).property("body").to.be.an("object");
            expect(response.body).property("data").to.be.an("object");
            expect(response.body.data).property("Demonstration").to.be.an("object");
            expect(response.body.data.Demonstration)
                .property("DemonstrationSkills")
                .to.be.an("array")
                .that.has.length(1);
