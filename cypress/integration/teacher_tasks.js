describe("Teacher tasks test", () => {
    ...
    it("View single tasks as teacher", () => {
        ...
        cy.get(".slate-appcontainer-bodyWrap .slate-placeholder").contains(
            "Select a section to load tasks dashboard"
        );
    ...
    it("Archive task as teacher", () => {
        ...
        cy.withExt().then(({ Ext, extQuerySelector, extQuerySelectorAll }) => {
            ...
            cy.wait("@taskData").should((xhr) => {
                expect(xhr.status).to.equal(200);
                expect(xhr.response.body.success).to.be.true;
            });

            cy.get(".slate-window").contains("Archive Task").click({ force: true });

            cy.wait("@taskSave").should((xhr) => {
                expect(xhr.status).to.equal(200);
                expect(xhr.response.body.success).to.be.true;
            });

            cy.get("#" + studentGrid.el.dom.id).contains("ELA Task One (archived)");
        });
    });
    ...
    it("Un-Archive task as teacher", () => {
        ...
        cy.withExt().then(({ Ext, extQuerySelector, extQuerySelectorAll }) => {
            ...
            cy.get(".slate-window").contains("Un-Archive Task").click({ force: true });
