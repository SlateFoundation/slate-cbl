...
folder are mapped to the application's root namespace, 'AggregridExample'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "AggregridExample"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'AggregridExample' in the same way
as `"AggregridExample/sass/src"`.
