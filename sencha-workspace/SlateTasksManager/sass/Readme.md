...
folder are mapped to the application's root namespace, 'SlateTasksTeacher'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "SlateTasksTeacher"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'SlateTasksTeacher' in the same way
as `"SlateTasksTeacher/sass/src"`.
